---
title: "Análise Exploratória de Dados no R Parte 2 - Soluções"
...
sol_cas %>% 
  group_by(EstadoCivil) %>% 
  summarize(N = n(), BaseCalculoMedia = mean(BaseCalculoMensal), Desvio_padrao = sd(BaseCalculoMensal)) 
...
```{r}
sol_cas_maior_2000 <- base %>% 
  filter(EstadoCivil == "solteir" | EstadoCivil == "casad", BaseCalculoMensal > 2000)


ggplot(sol_cas_maior_2000, aes(x = BaseCalculoMensal, fill = EstadoCivil, color = EstadoCivil)) + 
  geom_histogram(bins = 20, alpha=0.5, position="identity") 
```
...
base2000 %>% 
  group_by(sexo) %>% 
  summarize(N = n(), media = mean(BaseCalculoMensal), desvio_padrao = sd(BaseCalculoMensal))

ggplot(base2000, mapping = aes(x = BaseCalculoMensal, fill = sexo, color = sexo)) +
...
Note a diferença significante, entre os sexos, para a profissão de agente administrativo, haja vista o sexo masculino tem uma distribuição da base mais dispersa e com os quartis de valores mais elevados em relação à distribuição para o sexo feminino. 
